...
int main(void)
{
     ...
     */
    struct hashmap* hm = hashmap_create(BUCKETS_NUM, 
    ...
    for (i = 0; i < DB_SIZE; i++) {
        ...
        snprintf(db[i].name, KEY_SIZE, "item %d", i);

        /* Add current entry to hashmap */
        hashmap_set(hm, db[i].name, db[i].data);
    }
     ...
     */
    for (i = 0; i < DB_SIZE; i++) {
        if (hashmap_get(hm, db[i].name) != db[i].data) {
