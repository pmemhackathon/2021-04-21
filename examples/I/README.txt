...
new memory attributes: latency, capacity and bandwidth. We can use them to
allocate data used in different scenarios on different types of memory. Note,
that in this example, we don't explicitly say which kind of medium (e.g. DRAM or
PMEM) we want to use here, because some characteristics (like latency or
bandwidth) could be relative to relationship between source and destination
NUMA nodes. Instead of that we use a concept of "lowest latency" or "highest
bandwidth" nodes that could be accessed from a CPU that runs our program. This
concept was added to Memkind 1.11. To use it, a hwloc library must be installed
on the system with kernel >= 5.5. For more information about memory attributes,
see great https://pmem.io/2021/05/12/hmat_memkind.html article on PMEM.io blog.
